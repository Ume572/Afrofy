...
const NowPlayingScreen = ({ navigation, route }) => {
  ...
  // ✅ IMPROVED: Popup function with better message handling
  const showStylishPopup = (type, title, message) => {
    setPopupType(type);
    setPopupTitle(title);
    setPopupMessage(message);
  ...
  // ✅ IMPROVED: Like/Unlike handler with better error handling
  const handleLikeToggle = async () => {
    if (!songId) {
      showStylishPopup('error', 'Error', 'Song information not available');
      return;
    ...
    try {
      ...
      if (isLiked) {
        ...
        if (response && response.success) {
          ...
          // ✅ Agar already unliked hai toh info message dikhao
          if (response?.message?.includes('already') || response?.message?.includes('not liked')) {
            showStylishPopup('info', 'Already Removed', 'Song was already removed from likes');
            setIsLiked(false);
          } else {
        ...
        if (response && response.success) {
          ...
          // ✅ Agar already liked hai toh info message dikhao
          if (response?.message?.includes('already') || response?.message?.includes('already liked')) {
            showStylishPopup('info', 'Already in Likes', 'Song is already in your liked songs');
            setIsLiked(true);
          } else {
            throw new Error(response?.message || 'Failed to like song');
          }
      ...
      console.error('Like toggle error:', error);
      // ✅ Server error messages ko properly handle karo
      if (error.message?.includes('already') || error.message?.includes('Already')) {
        showStylishPopup('info', 'Already Processed', error.message || 'Song is already processed');
      } else {
        showStylishPopup('error', 'Error', 'Failed to update likes. Please try again.');
      }
    } finally {
  ...
  // ✅ IMPROVED: Add to playlist with better error handling
  const handleAddToPlaylist = async (playlistId, playlistName) => {
    if (currentlyAddingPlaylist) return;
    ...
    try {
      ...
      if (response.success) {
        ...
        // ✅ Agar song already playlist mein hai toh info message dikhao
        if (response.message?.includes('already') || response.message?.includes('Already') || response.message?.includes('exists')) {
          showStylishPopup('info', 'Already in Playlist', `"${displaySongData.title}" is already in "${playlistName}"`);
        } else {
          showStylishPopup('error', 'Error', response.message || 'Failed to add song');
        }
      }
  ...
  // ✅ IMPROVED: Popup Component with different types
  const StylishPopup = () => (
    <Modal
      visible={showPopup}
      transparent={true}
    ...
    >
      <View style={styles.popupOverlay}>
        <View style={[
          ...
          popupType === 'error' && styles.popupContainerError,
          popupType === 'info' && styles.popupContainerInfo
        ]}>
          <View style={[
            styles.popupIcon,
            popupType === 'error' && styles.popupIconError,
...
const styles = StyleSheet.create({
  ...
  // ✅ IMPROVED: Popup styles for different types
  popupOverlay: {
    flex: 1,
    backgroundColor: 'rgba(0, 0, 0, 0.8)',
    justifyContent: 'center',
  ...
  popupContainer: {
    ...
    maxWidth: wp(320),
    borderWidth: 2,
    borderColor: '#FF3B3B', // Default success color
  },
  popupContainerError: {
    borderColor: '#FF3B3B',
  },
  ...
  popupIcon: {
    marginBottom: hp(2),
    backgroundColor: 'rgba(175, 79, 76, 0.1)',
    borderRadius: wp(25),
    padding: wp(4),
  },
  popupIconError: {
  ...
  popupIconInfo: {
    backgroundColor: 'rgba(243, 37, 33, 0.1)',
  },
  popupTitle: {
    fontSize: wp(5),
    fontWeight: 'bold',
