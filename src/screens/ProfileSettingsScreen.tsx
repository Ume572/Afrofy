...
// src/screens/ProfileSettingsScreen.js - RESPONSIVE VERSION WITH STYLISH LOGOUT
import React, { useState, useEffect } from 'react';
import {
  View,
  Text,
...
const ProfileSettingsScreen = () => {
  ...
  // Custom logout modal states
  const [customLogoutModalVisible, setCustomLogoutModalVisible] = useState(false);
  ...
  // ✅ STYLISH LOGOUT FUNCTION - RED, WHITE & BLACK THEME
  const handleLogout = async () => {
    setCustomLogoutModalVisible(true);
  };

  const handleLogoutConfirmation = async (shouldLogout) => {
    setCustomLogoutModalVisible(false);
    ...
    <View style={styles.section}>
      ...
      <View style={styles.settingsCard}>
        ...
        >
          <View style={styles.settingLeft}>
            <View style={styles.settingIcon}>
              <Feather name="download" size={wp(5)} color="#FF3B3B" />
            </View>
            <View style={styles.settingText}>
              <Text style={styles.settingTitle}>Download Quality</Text>
              <Text style={styles.settingSubtitle}>{downloadQuality}</Text>
  ...
  const renderCustomLogoutModal = () => (
    <Modal
      visible={customLogoutModalVisible}
      transparent={true}
      animationType="fade"
      onRequestClose={() => setCustomLogoutModalVisible(false)}
    >
        ...
        <View style={styles.logoutModalContainer}>
          ...
          <View style={styles.logoutModalHeader}>
            <View style={styles.logoutIconContainer}>
              <Feather name="log-out" size={wp(8)} color="#fff" />
            </View>
          ...
          <View style={styles.logoutModalButtons}>
            ...
            >
              <Feather name="log-out" size={wp(4)} color="#fff" style={styles.logoutButtonIcon} />
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {renderQualityModal()}
      {renderLanguageModal()}
      {renderCustomLogoutModal()}
    </SafeAreaView>
  );
};

// ✅ RESPONSIVE STYLES WITH STYLISH LOGOUT
const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: '#000',
